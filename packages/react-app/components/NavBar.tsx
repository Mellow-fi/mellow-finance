...
export default function Header() {
  ...
  return (
    <Disclosure as="nav" className="bg-gray-800 border-b border-black">
      {({ open }) => (
        <>
          <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
            <div className="relative flex h-16 justify-between">
              <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
                {/* Mobile menu button */}
                <Disclosure.Button className="inline-flex items-center justify-center rounded-md p-2 text-white focus:outline-none focus:ring-1 focus:ring-inset focus:ring-white">
              ...
              <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                ...
                <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
                  ...
                  {isLoggedIn && (
                    <>
                      <Link to="/pools" className="text-white hover:text-yellow-400">
                        Pools
                      </Link>
          ...
          <Disclosure.Panel className="sm:hidden">
            <div className="space-y-1">
              ...
              {isLoggedIn && (
                <>
                  <Link to="/pools" className="block text-white hover:text-yellow-400">
                    Pools
                  </Link>
                  <Link to="/dashboard" className="block text-white hover:text-yellow-400">
                    Dashboard
                  </Link>
                  <button onClick={handleLogout} className="block text-white hover:text-yellow-400">
                    Logout
                  </button>
                </>
              )}
              {!isLoggedIn && (
                <Link to="/login" className="block text-white hover:text-yellow-400">
                  Login
                </Link>
              )}
            </div>
